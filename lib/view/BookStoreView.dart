...
class _BookStoreViewState extends State<BookStoreView> {
  ...
  Future getDashboardData() async {
    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await getVietJetDashboardDataRestApi().then((res) {
    ...
    //       mHeaderModel.clear();
    //       mCategoryList.clear();
              mNewestBookModel.addAll(dashboardResponse.data!);
  ...
  createHeaderData(bookModel, title, message, type) async {
    if (bookModel.length > 0) {
      String image1 = "";
      String image2 = "";
      if (bookModel[0].thumbnailUrl != null) {
        image1 = bookModel[0].thumbnailUrl.toString();
      }
      if (bookModel.length > 1) {
        if (bookModel[1].thumbnailUrl != null) {
          image2 = bookModel[1].thumbnailUrl.toString();
        } else {
          image2 = image1;
        }
      } else {
