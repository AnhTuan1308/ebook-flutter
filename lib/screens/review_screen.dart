...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  Future fetchData() async {
    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await getBookDetailsRestApi(widget.mBookId).then((res) {
          if (!mounted) return;
          setState(() {
            mIsLoading = false;
            BookResponse dashboardResponse = BookResponse.fromJson(res);
            mReviewList = dashboardResponse.bookdetail!.comments;
          });
        }).catchError((onError) {
          setState(() {
            mIsLoading = false;
                        ...
                        AnimationLimiter(
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              Comments reviewData = mReviewList![index];
