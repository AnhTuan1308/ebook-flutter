...
class _CategoriesListScreenState extends State<CategoriesListScreen> {
  ...
  List<Genres> mCategoriesList = [];
  List<Genres> mSearchList = [];
  ...
  Future getCategoriesList(page) async {
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await getCatListRestApi().then((res) async {
          setState(() {
            GenresResponse mCategory = GenresResponse.fromJson(res);
            appStore.setLoading(false);

            mCategoriesList = mCategory.genresdetail!;
            isLastPage = false;
                ...
                itemBuilder: (context, index) {
                  return CategoryListComponent(mSearchList[index]).paddingOnly(left: 16, right: 16).onTap(() {
                    ViewAllBooksScreen(
                      ...
                      categoryName: mSearchList[index].name.toString(),
