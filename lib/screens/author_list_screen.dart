...
class _AuthorListScreenState extends State<AuthorListScreen> {
    ...
    await isNetworkAvailable().then((bool) async {
      if (bool) {
        await getAuthorListRestApi(page, mPerPage).then((res) async {
          ...
          mAuthorList!.clear();
          AuthorResponseData authorResponseData = AuthorResponseData.fromJson(res);
          mAuthorList = authorResponseData.data;
          if(scrollController.position.pixels == index_reload){
              mSearchList!.clear();
          }
          if(mAuthorList!.isNotEmpty ) {
              mAuthorList!.forEach((element) {mSearchList!.add(element);});
          }
          else {
            return;
          }
  ...
  String? getAuthorName(authorListResponse) {
    return authorListResponse.fullName;
  }
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    searchWidget(context, controller: mSearchCont, hint: keyString(context, "lbl_search_by_author_name"), onChanged: (string) {
                      setState(() {
                        mSearchList = mAuthorList!.where((u) => (u.fullName!.toLowerCase().contains(string.toLowerCase()) || u.fullName!.toLowerCase().contains(string.toLowerCase()))).toList();
                      });
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return AnimationConfiguration.staggeredGrid(
                            ...
                            child: SelectedAnimationType(scale: 3, curves: getIntAsync(ANIMATION_TYPE_SELECTION_INDEX) == SLIDE_ANIMATION ? Curves.easeOut : Curves.easeOutBack, child: AuthorListComponent(mSearchList![index], bgColor)).onTap(() {
                              AuthorDetails(
                                mSearchList![index].imageUrl.validate(),
